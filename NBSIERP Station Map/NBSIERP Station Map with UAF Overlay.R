...
#note, this is installed from AFSC-GAP github along with akgfmaps
#library(coldpool)
...
uaf <- read.csv("data/UAF_Coordinates.csv")
#temp data
...
plot <- ggplot() +
 # geom_tile(data=nbs_19,aes(x=lon,y=lat,fill=btm_temp)) +
  ...
  geom_point(data=uaf,aes(x=Longitude,y=Latitude,shape=Label,color=Label,size=Label),alpha=0.6) +
  ...
  labs(x="Longitude (°W)",y="Latitude (°N)",color="Station Type",shape="Station Type",size="Station Type",title="Original Proposed Locations") +
  ...
  scale_color_manual(values=c("NBS Surface Trawl Station"="black","Moorings"="gold","IERP Station" = "orangered", 
                              "UAF Station" = "blue", "DBO" = "green")) +
  scale_shape_manual(values=c("NBS Surface Trawl Station"=19,"Moorings"=17,"IERP Station"=15,
                              "UAF Station" = 19, "DBO" = 20)) +
  scale_size_manual(values=c("NBS Surface Trawl Station"=5,"Moorings"=7,"IERP Station"=5,
                             "UAF Station" = 4, "DBO" = 5)) +
  theme(axis.ticks=element_blank()) +
  theme(panel.grid=element_line(color=rgb(235, 235, 235, 100, maxColorValue = 255),size=0.5,linetype=1)) +
  theme(panel.background=element_rect(fill="white"))

ggsave(plot=plot,filename="figures/NBSIERP UAF Survey Map.jpg",width=8,height=8,units=c("in"),dpi=300)
